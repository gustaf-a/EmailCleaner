...
public class SortBySender_should
{
    ...
    {
        var sortBehavior = new SortBySender();

        Assert.Equal("SortBySender", sortBehavior.GetName());
    }
    ...
    public void Sort_into_sortedEmails()
    {
        ...
        {
            var distinctSenders = emailGroup.Emails
                                    .Select(e => e.SenderAddress).Distinct();

            Assert.Single(distinctSenders);
        }
    }
}
